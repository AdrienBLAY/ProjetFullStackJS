          ...
          .then(function(response) {
            if(response.error.error)
              Alert.error(response.error.message);
                ...
                Alert.success("L'article a bien été modifié");
                hashHistory.push('article/'+that.props.params.articleId);
            }
          });
    }
    else {
      Alert.error("Le Body, le Titre, le Topic et le changement d'info sont requis.");
    }
  }
    ...
    if(this.state.loading)
      ...
      return (
        <div className="new-article">
          ...
          <trix-editor toolbar="my_toolbar" input="my_input" placeholder="Commencé à écrire ici...." class="input-body"></trix-editor>
          <input id="my_input" type="hidden" value={this.state.body} ref="body" onChange={this.handleChange}/>
                 <br/>
                 <label>Choisissez un topic</label>
                 ...
                 <label>Quel changement avez vous fait ?</label>
                 <textarea
                   ref="what_changed"
                   className="form-control what_changed what"
                   id="what"
                   placeholder="Example : Modifier une type, Faute de frappe, etc."
                    />
                  </div>
                  <br/>
            </div>
        ...
        <div className="row">
          <div className="col-md-12">
            <button className="btn btn-default btn-block btn-lg" onClick={this.handleSubmit}>Modifier</button>
          </div>
        </div>
      </div>
      </div>
